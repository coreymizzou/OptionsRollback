...
def compute_trade_pricing(trade: dict) -> dict:
    ...
    if "SPREAD" in strategy:
        # Validate long leg has live bid/ask before computing debit
        long_bid = main.get("bid", 0) or 0
        long_ask = main.get("ask", 0) or 0
        if long_bid > 0 and long_ask > 0:
            main_mid = round((long_bid + long_ask) / 2, 2)
            main["mid"] = main_mid  # override with fresh mid
